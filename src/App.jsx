...
const App = () => {
  ...
  const [installCost, setInstallCost] = useState("5,000,000");
        ...
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm space-y-3 transition-all hover:border-blue-400">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 p-1 bg-slate-50 rounded-lg group-hover:bg-blue-50 transition-colors pointer-events-none border border-slate-100 shadow-sm">
                    <ChevronDown className="w-6 h-6 text-slate-900" strokeWidth={3} />
                  </div>
                </div>
              </div>

              <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm space-y-3 transition-all hover:border-blue-400 group">
                <label className="text-[13px] font-black text-slate-700 uppercase tracking-tight flex items-center gap-2">
                  저장 기간 <span className="text-[10px] text-slate-300 font-bold">(일)</span>
                </label>
                <div className="flex items-center gap-2">
                  <input type="text" inputMode="numeric" value={storageDays} onChange={handleInputChange(setStorageDays)}
                    className="w-full text-3xl font-black border-b-2 border-slate-50 focus:border-blue-500 outline-none pb-1" />
                </div>
              </div>

              <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm space-y-3 transition-all hover:border-blue-400 group">
                <label className="text-[13px] font-black text-slate-700 uppercase tracking-tight flex items-center gap-2">
                  저장 물량 <span className="text-[10px] text-slate-300 font-bold">(kg)</span>
                </label>
                <div className="flex items-center gap-2">
                  <input type="text" inputMode="numeric" value={storageWeight} onChange={handleInputChange(setStorageWeight)}
                    className="w-full text-3xl font-black border-b-2 border-slate-100 focus:border-blue-500 outline-none pb-1" />
                </div>
              </div>

              <div className="p-5 bg-white border border-slate-200 rounded-2xl shadow-sm space-y-3 transition-all hover:border-blue-400 group">
                <label className="text-[13px] font-black text-slate-700 uppercase tracking-tight">도매 기준가 <span className="text-[10px] text-slate-300 font-bold">(원)</span></label>
                <div className="flex items-center gap-2">
                  <input type="text" inputMode="numeric" value={wholesalePrice} onChange={handleInputChange(setWholesalePrice)}
                    className="w-full text-3xl font-black bg-transparent border-b-2 border-slate-50 focus:border-blue-500 outline-none pb-1" />
                </div>
              </div>
            </div>
          </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="rounded-[2.5rem] border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col transition-all hover:shadow-md">
                ...
                <div className="p-10 space-y-5 flex-grow">
                  ...
                  <div className="flex justify-between items-center text-sm font-bold text-slate-500">
                    <span>예상 감모율</span>
                    <span className="text-slate-900">{formatFloat(calcResults.convertedBaseLoss)}%</span>
                  </div>
                  <div className="flex justify-between items-center text-sm font-bold text-slate-500">
                    <span>감모 및 폐기량</span>
                  ...
                  <div className="flex justify-between items-center text-sm font-bold text-slate-500">
                    <span>손실 추정액</span>
                    <span className="text-red-500">- ₩ {formatNum(calcResults.baseWasteCost)}</span>
                  </div>
                  <div className="pt-6 border-t-2 border-slate-900">
                    <span className="block text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest uppercase">실수령 금액 (저장금액-감모금액)</span>
                    <div className="text-4xl font-black text-slate-800 tracking-tighter italic">₩ {formatNum(calcResults.baseActualValue)}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[2.5rem] border-2 border-blue-600 bg-white shadow-2xl shadow-blue-100 overflow-hidden flex flex-col transition-all hover:scale-[1.01]">
                ...
                <div className="p-10 space-y-5 flex-grow">
                  ...
                  <div className="flex justify-between items-center text-sm font-bold text-slate-400">
                    <span>예상 감모율</span>
                    <span className="text-blue-600 font-bold">{formatFloat(calcResults.convertedAirLoss)}%</span>
                  </div>
                  <div className="flex justify-between items-center text-sm font-bold text-slate-400">
                    <span>감모 및 폐기량</span>
                  ...
                  <div className="flex justify-between items-center text-sm font-bold text-slate-400">
                    <span>손실 추정액</span>
                    <span className="text-blue-600 font-bold">- ₩ {formatNum(calcResults.airWasteCost)}</span>
                  </div>
                  <div className="pt-6 border-t-2 border-blue-600">
                    <span className="block text-[10px] font-black text-blue-300 uppercase mb-2 tracking-widest text-right">실수령 금액 (저장금액-감모금액)</span>
                    <div className="text-4xl font-black text-blue-600 tracking-tighter italic text-right">₩ {formatNum(calcResults.airActualValue)}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-slate-200 shadow-xl overflow-hidden transition-all">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="space-y-8 md:pr-10 md:border-r border-slate-100 pb-8 md:pb-0">
                  ...
                  <div className="space-y-8">
                    <div className="space-y-4">
                      <div className="flex items-center gap-2">
                        ...
                        <label className="text-sm font-black text-slate-500 uppercase tracking-widest">에어조나 기기 설치 비용</label>
                      </div>
                      <div className="flex items-center gap-3 border-b border-slate-200 focus-within:border-blue-600 transition-colors py-1 group">
                         <span className="text-xl font-bold text-slate-400 group-focus-within:text-blue-600">₩</span>
                         <input 
                            type="text" 
                            value={installCost} 
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-blue-600 rounded-full" />
                        <label className="text-sm font-black text-slate-500 uppercase tracking-widest">연간 예상 저장 회수</label>
                      </div>
                      <div className="flex items-center gap-4 border-b border-slate-200 focus-within:border-blue-600 transition-colors py-1 group">
                         <input 
                            type="text" 
                            inputMode="numeric" 
                            value={annualCycles} 
                  ...
                  <div className="p-5 bg-slate-50 rounded-2xl flex items-start gap-4 border border-slate-100 shadow-inner">
                     <Info className="w-5 h-5 text-slate-400 mt-0.5" />
                     <p className="text-xs text-slate-500 font-bold leading-relaxed">
                       연간 예상 저장회수를 수정하여<br/>도입 효과를 실시간으로 분석하세요.
                     </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col space-y-6 justify-center">
                  ...
                  {/* 연간 기대 절감액 */}
                  <div className="bg-slate-50/50 rounded-3xl p-6 border border-slate-100 space-y-2 group transition-all hover:bg-white hover:shadow-md">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-[0.1em]">연간 총 기대 절감액</p>
                  ...
                  <div className="bg-blue-600 rounded-3xl p-8 shadow-lg relative overflow-hidden group transition-all hover:scale-[1.02]">
                    ...
                    <div className="relative z-10 space-y-2">
                      ...
                      <h4 className="text-lg font-bold text-white mb-2">투자비용 회수기간</h4>
